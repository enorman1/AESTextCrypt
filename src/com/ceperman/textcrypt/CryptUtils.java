...
public class CryptUtils {
	...
	public static CipherInfo createCiphers(byte[] password, byte[] salt, int rounds, int keylength) {
		...
		try {
			...
			int plaintext[] = { 0x155cbf8e, 0x57f57513, 0x3da787b9, 0x71679d82, 0x7cf72e93, 0x1ae25274, 0x64b54adc,
				0x335cbd0b };
			if (keylength < 256) {
				plaintext = Arrays.copyOf(plaintext, keylength / 32); // shorten the key
			}
			BCrypt bcrypt = new BCrypt();
			...
			byte[] rawKey = bcrypt.crypt_raw(passwordWithDelim, salt, rounds, plaintext );
			SecretKeySpec aesKey = new SecretKeySpec(rawKey, "AES");
			// TODO : IV historique a reproduire ici
			byte[] iv = new byte[16];//getLegacyIv(salt, rounds, keyLength);
			IvParameterSpec ivSpec = new IvParameterSpec(iv);
			Cipher enc = Cipher.getInstance(LEGACY_TRANSFORMATION); // "AES/CBC/PKCS5Padding"
			enc.init(Cipher.ENCRYPT_MODE, aesKey, ivSpec);
			Cipher dec = Cipher.getInstance(LEGACY_TRANSFORMATION); // "AES/CBC/PKCS5Padding"
			dec.init(Cipher.DECRYPT_MODE, aesKey, ivSpec);
			info.encryptCipher = enc;
			info.decryptCipher = dec;
			info.salt = salt;
			info.iv = iv;
			info.rounds = rounds;
	...
	*/
	...
	/* determine max key length allowed by security policy */
		...
		return Cipher.getMaxAllowedKeyLength(LEGACY_TRANSFORMATION);
	}
